...
public class AnalyzeCodeJava implements AnalyzeCodeStrategy {

    public static final int JAVA_MAX_METHOD_LINES = 20;
    public static final int JAVA_MAX_LINE_CHARACTERS = 80;
    ...
    private void checkIfLinesAreEquals(List<String> lines, Set<LinterError> linterErrors, int i, int j) {
        ...
        if (linesAreEquals(currentLine, otherLine)) {
            ...
            if (JAVA_KEYWORDS.contains(currentLine)) {
    ...
    private void checkMethodLength(List<String> lines, List<LinterError> linterErrors, int currentLineNumber) {
        ...
        if (methodLines > JAVA_MAX_METHOD_LINES) {
            linterErrors.add(
                    new LinterError()
                            .setErrorMessage(
                                    String.format("Method %s contains more than %s lines (%s)",
                                            getMethodName(line), JAVA_MAX_METHOD_LINES, methodLines))

                            .setErrorNumber(methodLines / JAVA_MAX_METHOD_LINES)
            );
        }
    }
    ...
    private void checkLineLength(String line, int lineNumber, Set<LinterError> linterErrors) {
        ...
        if (lineLength > JAVA_MAX_LINE_CHARACTERS) {
            ...
            var newError = new LinterError()
                    ...
                    .setErrorMessage(String
                            .format("Line %s contains more than %s characters (%s)",
                                    lineNumber, JAVA_MAX_LINE_CHARACTERS, lineLength));
    ...
    private int calculateErrorNumber(int lineLength) {
        var errorNumber = 1;
        lineLength = lineLength - JAVA_MAX_LINE_CHARACTERS;
